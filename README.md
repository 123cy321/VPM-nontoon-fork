...
> **定位（2026-09-18 起）**：**轻量化的真实光影** —— 以**真实光照**（自带光 / 世界光）为核心，
> 阴影能力按"**阶梯**"逐级加强、**默认零开销**（当前已发布的是阶梯的**默认级**）。
> ⛔ **本分支不是"第二个 lilToon"**：不追求补齐 lilToon 的功能清单；lilToon 素材的**一键迁移**
> 是入口，不是终点。
...
- **软影宽度与烘焙分辨率无关**（着色器 **0.3.9** 起）：以前半径以纹素计算，分辨率越高半影越窄
  （实测同一场景 256² → 512² 世界尺度只剩 71%、1024² 只剩 57%）；0.3.9 起按分辨率换算半径、
  并同步放大采样数，256²/512²/1024² 的世界宽度已一致（实测 86%）。
  ⇒ **不必**为了软影去调烘焙分辨率；且 **256² 的输出与旧版逐像素一致**（存量 avatar 零变化）
