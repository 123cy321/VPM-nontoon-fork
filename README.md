...
- **软影宽度与烘焙分辨率挂钩**：半径以纹素计算，分辨率越高半影越窄（实测 256² → 512²，世界尺度只剩 71%）。
  换了烘焙分辨率后请重新调「柔度」
- **默认「柔度 0.5」的柔化量很小**：真实尺度（例如直径 0.3 m 的头部、遮挡物在前 5 cm）下半径会落到
  1.5 纹素的下限，实测只比硬阴影柔化约 2.5 mm。想要明显的软影，请把「柔度」拉到 1 并提高烘焙分辨率
...
- 自有光源的「柔度」没有物理量纲：实际的半影宽度取决于遮挡物与接收面的距离比**以及**烘焙分辨率，
  因此换分辨率或换模型后都需要重新调（数值关系见 5.3）。真实尺度下默认档的柔化量只有毫米级
