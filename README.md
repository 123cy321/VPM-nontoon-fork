...
| 当前版本 | 着色器 **0.3.12** ／ 工具包 **0.5.4**（2026-09-18 起改用自有包 id，见 §8） |
...
| **T1** | 原生阴影外围增强：**锐化** + **环境光染色** | 零～极低 | 已发布 |
| **T2** | `fwidth` 解析分界 + 抗锯齿 | 低（无采样） | 未实现 |
| **T3** | PCSS 实时半影 | 高（20–96 采样/像素） | 路线保留，当前无实现（深度来源未决） |

> **本手册对应的已发布版本**：着色器 **0.3.12** ／ 工具包 **0.5.4**。表中标注**未实现**的层级
> （**T2** / **T3**）**尚不可用**，请勿当作已具备。
>
