...
| 版本 | 要点 |
|---|---|
| 0.3.11 / 工具 0.5.3 | **与上游彻底分割身份**：着色器包 id → `com.catandling.nontoon`，工具包 → `com.catandling.nontoon-converter`，着色器名 → `nontoon-fork` / `nontoon-fork-fur` / `nontoon-fork-twopass`；VPM 仓库 id → `io.github.catandling.vpm`（URL 未变）。**程序集名与资产 GUID 保留**（实测：与官方 0.1.3 共有的 78 个 `.meta` GUID 逐字符相同）⇒ 已有材质、预制体、场景不断引用；但**没有迁移路径**，一律按**全新安装**处理，且**装之前必须先删掉旧包**（同 GUID 的资产会被导入两次）。**着色器侧移除自渲线性深度 PCSS 路径**：15 个 `_SelfLightRt*` 属性与自渲采样代码已删（实测：0.3.10 的 zip 里 `_SelfLightRt` 命中 **34** 处，0.3.11 为 **0** 处）。活跃的实时自阴影是「自带光源 → **Unity 为该光源渲染阴影贴图** → 着色器采样该贴图」（见 `NonToon/SCOPE.md`）。⚠️ **已知不一致（已如实记录，未隐藏）**：`Shaders/Modules/SelfLight/NTRTDepth.shader` 仍在包内，且工具包 `Runtime/NTSelfRealtimeShadow.cs` 仍在 `Shader.Find("NonToon/NTRTDepth")` 调用它 —— `SCOPE.md` 写明工具包那半个「待移除」。工具包 **0.5.3**：身份改名 + **[NT-FIX 31] 渲染队列兜底修复**（脚本/撤销/粘贴/第三方工具改 `_RenderingMode` 会让 `renderQueue` 陈旧，实测两个方向都错；新增命令按渲染模式重新对齐，幂等）+ **[NT-FIX 35] 缺失能力诊断修正**（按真实素材频次给专门、可定位的警告，不再用一条笼统文案宣称覆盖一切）+ 期望着色器版本对齐 0.3.11。另：**干净工程从零安装实测通过**（用 vrc-get 对 listing 真解析依赖，5 个包全解开） |
