...
**一句话目标：轻量化的真实光影。** 以**真实光照**（自带光 / 世界光驱动明暗，而不是把光照烘死）为核心，
阴影能力按"**阶梯**"逐级加强、**默认零开销**。差异化对象是 **lilToon 那条"全功能 + 真实光影"的重路线**
（性能影响过大）。⛔ **本分支不是"第二个 lilToon"**：不追求补齐 lilToon 的功能清单；
lilToon 素材的**一键迁移**是入口，不是终点。

阶梯 = **能力 × 代价**：越高一级，收益越强、性能占用越大；用户按自己的需求与预算选一级。

| 级 | 内容 | 性能占用 | 现状（本手册对应版本） |
|---|---|---|---|
| **T0** 默认 | 上游原生 ramp（`_ShadeGradientIndex` 默认 `-1` ⇒ 默认**关闭**，这是上游原版行为） | 零新增采样 | 已发布 |
| **T1** | 原生阴影外围增强：**锐化** + **环境光染色** | 零～极低 | 未发布（0.3.12 待发） |
| **T2** | `fwidth` 解析分界 + 抗锯齿 | 低（无采样） | 未实现 |
| **T3** | PCSS 实时半影 | 高（20–96 采样/像素） | 路线保留，当前无实现（深度来源未决） |

> **本手册正文只描述已发布的版本**：着色器 **0.3.11** ／ 工具包 **0.5.3**。表中标注**未发布 / 未实现**的层级
> **尚未发布或尚未实现**，请勿当作已可用；未发布内容的清单见 [附录 A](#附录-a-版本记录) 的「待发布」条目。
>
> 实时阴影相关的统一口径：**烘焙自阴影（②/④）可用；实时自阴影（⑥）当前没有可用实现，其深度来源未决**
> （见 §6）。

## 文档导航

- [定位与能力阶梯](#定位与能力阶梯) —— 本分支的目标、T0–T3 能力阶梯与当前可用性
...
> ⚠️ **装之前先删掉旧的 NonToon**：官方版，或本分支 **0.3.10 及更早**的包
> （旧 id 是 `jp.lilxyzw.nontoon` / `com.123cy321.nontoon-converter`）。2026-09-18 起本分支改用
> **自有包 id**，VPM **不会**把旧包当成同一个包去替换；而本分支保留了官方 0.1.3 的资产 GUID ⇒
> 两份同时存在会让 Unity 把同一批 GUID 导入两次。详见 12.1。

## 2. 五分钟上手

### 2.1 界面语言

0.3.7 起无需任何操作，中文会自动生效：本包会把中文语言表补齐成**当前语言**的那一份
（中文 Windows 上 ShaderCore 的默认语言是 `zh-CN`；机制细节见 §9），
165 条面板属性中 117 条显示中文。
...
三种方案的详细参数与适用场景见下一节；**性能代价的完整口径见 §5**。
...
代价：占用 VRChat 性能等级的 `Lights` 计数（**挂一盏实时光后性能等级最高只能到 Poor**）、
需额外渲染一张阴影贴图、Quest 端基本不可用 —— **完整口径见 §5.2**。
...
- **这路光只作用于本材质**：不会照亮世界，也不会照亮其他玩家
- **所有人看到的是同一份效果**：光的方向/颜色/强度、以及 PCSS 自阴影，都烘焙进材质与深度图，随 avatar 一起上传
  ⇒ 别人同样看得到你的自有光与自阴影（这一点与上面的 ① Avatar 光源相反：那盏实时光只有本地玩家自己的客户端在照自己）
- 阴影来自**烘焙的深度图**（非实时阴影），**不依赖观看者的阴影设置**，始终可见；Quest 上可用（PCSS 关闭时）
- 代价：自有光源不占用 VRChat 的 `Lights` 计数，采样数按档位计，见 §5.1
...
> 影子对应的是**烘焙那一刻的姿态**，更换姿势后重新烘焙即可。
> 为什么 ②/④ 这条路径只能烘焙：这里的"私有光"是**着色器内部**的一路光，Unity 不会为它渲染
> 阴影贴图；而 VRChat 里 avatar 不能带相机与渲染纹理 ⇒ 没有实时重算这张深度图的办法。
> 需要**实时**阴影请走 **① Avatar 光源**（真挂一盏 Unity 实时光，由 Unity 渲染它的阴影贴图），
> 代价是占用 VRChat 的 `Lights` 计数，见 3.2。
>
> ⚠️ **实时自阴影（⑥）当前没有可用实现，其深度来源未决**（见 §6）。上面 ②/④ 的**烘焙**自阴影不受影响。
...
| **只想最快搞定：不摆灯、不碰材质面板** | **自带光照与阴影（④）** |
| 只用 NonToon，地图有时很黑 | 先做亮度预设（③，零代价），仍不满意再加自有光源 |
| 在意性能等级，或要上 Quest | 亮度预设 + 自有光源/自带光照（烘焙阴影）；不要用 ① 的实时光源 |
| 希望玩家自己能调亮度 | 用 ③ 生成径向参数 |
| 需要 avatar 在任何地图里观感完全一致 | ② 或 ④，并勾选「只由它照亮」 |

### 3.6 自带光照与阴影（不需要地图光源）

菜单：`GameObject/NonToon/④ 自带光照与阴影（不需要地图光源）`

这是「一键模式」：**不用往场景里摆任何灯**，也不用逐项调材质面板。组件自带一盏**虚拟光源**，
把光照与自阴影一次配置好：

| 组件里有什么 | 说明 |
|---|---|
| 环绕角 / 俯仰角 | 光从哪个方向来（俯仰角 +90 = 头顶正上方）；面板实时显示换算出的方向向量 |
| 颜色 / 色温 / 强度 | 与 ② 同一套参数 |
| 只由它照亮 | 默认**开启**：丢弃世界光、环境光、光照贴图、天空盒反射 ⇒ avatar 在任何地图里观感一致 |
| 同时抬高亮度下限（防煤） | 默认**开启**，下限 0.35 ⇒ 全黑地图里不会变成一块煤（零采样代价） |
| 自阴影 | 用上面的虚拟光方向烘焙深度图；PCSS 档位、柔度、浓度、硬化、距离、接收遮罩、分辨率都在面板上 |

点「一键配置并烘焙」即可。数据全部写进材质，**运行时组件没有任何行为**，烘焙完可以删掉它。

**它和 ② 的关系**

| | ② 自有光源（手动） | ④ 自带光照与阴影（一键） |
|---|---|---|
| 光源来自 | 场景里的一盏 `Light`（`Light Source` 字段） | **组件上的虚拟方向**（不需要任何灯） |
| 亮度下限 | 需要自己去 ③ 或材质面板设 | 组件里一并设好 |
| 适合 | 想精细控制、或已有指定的灯 | 想最快搞定 |

两者写的是同一批材质属性，**不要在同一个目标上同时挂**（面板会检测并警告）。

**实测**：组件用虚拟光烘焙的结果，与「用真 `Light` 指定同一方向」烘焙的结果**逐像素一致**
（1 万 6 千个像素最大差 0.00001，无一超过 1/255）——所以「不需要场景里的灯」是**等价替换**，
而不是另写了一套会漂移的实现。

**与常用改模工具的配合**：被 PhysBone 驱动、或被 `MA WorldFixedObject` 固定到世界的部件不要参与烘焙；
用 `MA Scale Adjuster` / `Merge Armature` 调过比例位置的 avatar 请先调完再烘焙；
混用 lilToon / Poiyomi 时本组件只对 NonToon 部位生效（面板会列出被跳过的材质）。

### 3.7 亮度调整（构建时固定 / 游戏内可调）

菜单：`GameObject/NonToon/⑤ 亮度调整（构建时固定 / 游戏内可调）`

③ 是编辑器窗口（一次性把值写进材质）；⑤ 是**组件**，把同样的事变成"放上去、点一下"，并且多了一条
**非破坏**路线。同一个组件两种用法：

| 用法 | 做什么 | 代价 |
|---|---|---|
| **构建时固定** | 直接把材质属性写成固定值（默认 `_LightMinLimit = 0.35`） | 零 Animator、零参数、不需要 VRCSDK |
| **游戏内可调** | 生成两条曲线剪辑 + 一个控制器，玩家在 Action Menu 里用径向随手调 | 一个 Float 参数（8 bit 同步内存） |

**装了 Modular Avatar 就走非破坏路径**（自动判断，面板会写明走的是哪条）：

| | 有 Modular Avatar | 没有 Modular Avatar |
|---|---|---|
| 生成的资产 | 曲线剪辑 + **独立**控制器 | 曲线剪辑 + 控制器 |
| FX 层 | MA 的 `Merge Animator` 在构建期合并 | 直接写进 avatar 现有的 FX 控制器 |
| 参数 / 菜单 | MA 的 `Parameters` + `Menu Item`(径向) + `Menu Installer` | 直接改 `ExpressionParameters` 与 `ExpressionsMenu` |
| 回滚 | 删掉组件即可（你原有的资产一个字节都没动） | 需要手动清理 |

实测（真实 Modular Avatar 1.18.1 + NDMF 完整构建，45 项断言全过）：生成后你原有的
`ExpressionParameters` / `ExpressionsMenu` **仍为 null**（我们没写）；构建后我们的动画层、
Float 参数、径向菜单项**全部存活**，材质属性曲线也仍然指向 avatar 上的正确对象。

驱动属性可以是 `_LightMinLimit`（防煤主力）、`_LightMaxLimit`（压过曝）或 `_SelfLightIntensity`
（自有光强度），也可以填自定义属性名 —— 所以**同一个组件也能给 lilToon 材质调亮度**（两者亮度公式一致）。

> 与 AAO 的关系：曲线对每个 Renderer 写**同一个值**，这正是 AAO 网格合并支持的形式
> （0.4.8 起已用真实 AAO 1.9.16 跑过 23 项断言）。
> 若同时用 VRCFury 的 `Full Controller` 接管 FX 层，两者都能工作，但别重复控制同一层。
...
**① Avatar 光源**（真挂一盏 Unity 实时光，见 3.2）：
...
- 第一种（Avatar 光源）与第二种（自有光源）**不要同时开启**，会双份打光。
  第二种组件的实时光源模式会自动关闭材质里的自有光源并给出警告；⚠️ 但**实时自阴影（⑥）当前没有
  可用实现，其深度来源未决**（见 §6）—— 可用的自阴影一律是**烘焙**路径（②/④）
- 自有光源的阴影是静态的：更换姿势或服装后需重新烘焙
- **软影宽度与烘焙分辨率无关**（着色器 **0.3.9** 起按分辨率换算半径、并同步放大采样数）；
  256²/512²/1024² 的世界宽度已一致（实测 86%）。⇒ **不必**为了软影去调烘焙分辨率；
  且 256² 的输出与旧版**逐像素一致**（存量 avatar 零变化）
- **默认「柔度 0.5」的柔化量很小**：真实尺度（例如直径 0.3 m 的头部、遮挡物在前 5 cm）下半径会落到
  1.5 纹素的下限，实测只比硬阴影柔化约 2.5 mm。想要明显的软影，**请把「柔度」拉大**
  （真实尺度下半影本来就只有毫米级 —— 这是「柔度」无量纲、绝对量小导致的，见下面「已知限制」）
- **「阴影偏移」是包围盒深度的比例**（默认 2%），它的世界含义会随包围盒深度变化：包围盒 2.1 m 时约 4.2 cm、
  头部尺度（0.42 m）时约 8 mm。同一个数值在不同烘焙之间表现不同。
  「Shadow Texels」是纹素↔UV 的换算基准，**必须等于阴影贴图的分辨率**，不要手改
- **参与烘焙的部件不要被动态驱动**：自阴影的纵深方向是烘焙那一刻固定的，
  头发/欧派等被 PhysBone 驱动、或被 `MA WorldFixedObject` 固定到世界的部件，影子不会跟着动
  —— 这类部件建议排除出烘焙，只烘焙相对静止的部分（脸、躯干）
- **用 MA 调过比例/位置就先调完再烘焙**：`MA Scale Adjuster` / `MA Merge Armature` / `MA MoveTo` 会在
  **构建时**改变 transform，烘焙会把这些变化固化进材质里的世界空间基
...
- **⑤ 亮度调整会优先走 MA 非破坏路径**（检测到 MA 时）：只生成独立资产 + 挂
  `Merge Animator` / `Parameters` / `Menu Item` / `Menu Installer` 四个组件，
  不修改你原有的 FX 控制器、`ExpressionParameters` 与菜单。删掉组件即回滚
- 参数名建议保持 `NT_` 前缀（默认就是 `NT_Brightness`），避免与别的改模工具撞名
- `Remove Vertex Color`：若同时开启了「从顶点色取描边宽度」（默认关闭），顶点色被移除后描边方向会走形
- `Material Swap / Material Setter`：可以共用。运行时亮度径向仍会写当前材质的属性，
  但换上的材质不会保留其面板中的亮度值
- 用 `MA Scale Adjuster` / `Merge Armature` / `MoveTo` 调整过比例位置的 avatar：
  **先调整完再烘焙自阴影**（② / ④），否则材质里的世界空间基与最终姿态不符
- 自阴影的纵深是烘焙那一刻固定的：被 **PhysBone 驱动**或被 **`MA WorldFixedObject` 固定到世界**
  的部件不要参与烘焙
- 没有装 MA 时，⑤ 与 ③ 走同一条"直接写入"路径（会改你的 FX 控制器与参数/菜单），面板上会明确提示
...
| 实时光源 | 基本不可用 |
| 自有光源的烘焙自阴影 | 可用（**PCSS 关闭**时是 1 次采样、无循环；打开 PCSS 后是动态循环，Quest 上未实测）|
| 插件生成的 Animator 参数 | 可用（均为白名单内资产） |

## 6. 已知限制

- **只在 BiRP（VRChat）上保证可用**。URP 分支在 Unity 2022.3 上属于不生效的代码路径
- **② / ④ 的「私有光 + 遮盖自阴影」只能烘焙**：着色器内部的光 ⇒ Unity 不为它渲染阴影贴图；
  avatar 不能带相机与渲染纹理 ⇒ 无法实时重算深度图。影子固定为"烘焙那一刻的姿态"。
  需要**实时**阴影时用 **① Avatar 光源**（Unity 渲染该光的阴影贴图），代价是占用 `Lights`
  计数、PC 端性能等级最高只能到 Poor（见 §5.2）
- **实时自阴影（⑥）当前没有可用实现，其深度来源未决**：着色器里**没有**任何"采样私有光 Unity
  阴影贴图"的代码路径，也没有自渲线性深度通路；候选的两种深度来源（Unity 阴影贴图 / 自渲线性深度图）
  尚未定案。**烘焙自阴影（②/④）可用**，只是形状固定为烘焙那一刻的姿态
...
- 自有光源的「柔度」没有物理量纲：实际的半影宽度取决于遮挡物与接收面的距离比（**不含**烘焙分辨率
  —— 0.3.9 起分辨率已不影响世界宽度）。换模型/换姿态后仍需重新调（数值关系见 5.3）。
  真实尺度下默认档的柔化量只有毫米级
...
**会并存**，而且这正是 2026-09-18 起的设计：本分支已改用**自有包 id** `com.catandling.nontoon`
与**自有着色器名** `nontoon-fork`（官方是 `jp.lilxyzw.nontoon` / `NonToon`）⇒ 两者互不覆盖，
也不会争同一个着色器名。

⚠️ **但装之前请先把旧的 NonToon 删掉**（官方版，或本分支 0.3.10 及更早的版本）：
本分支**保留了**官方 0.1.3 的资产 GUID（目的是让已有材质的引用不断），两份同时存在
会让 Unity 把同一批 GUID 导入两次，产生重复资产。
...
旧版本的着色包内嵌了转换器。请把包目录整个删掉后重装一次。旧路径是
`Packages/jp.lilxyzw.nontoon` 与 `Packages/com.123cy321.nontoon-converter`；
2026-09-18 起的新路径是 `Packages/com.catandling.nontoon` 与
`Packages/com.catandling.nontoon-converter`。

**问：该用哪个 Unity 版本？**
**2022.3.22f1**。本包就是在这个版本上开发与验证的（`_verify-proj` 与全部探针都在 2022.3.22f1 上跑），
社区通行的改模教程（例如 VRCD 文档库的《VRC改模从入门到入门》）也推荐 2022.3.22f1。
换用其他 2022.3.x 小版本一般没问题，但换大版本（2021 / 6.x）本包未做验证。
...
- **VPM 仓库 id 现为 `io.github.catandling.vpm`**（2026-09-18 起，与包 id、着色器名同批切分，
  目的是彻底摆脱旧账号名）。仓库 **URL 未变**（`https://catandling.github.io/VPM-nontoon-fork/vpm.json`）。
  **已接受的代价**：已添加过本仓库的用户，客户端里可能多出一条失效条目，需手动删掉旧的（见文首「关于账号改名」）
- **工具包 0.5.2 起 Modular Avatar 是必需依赖**：需要菜单的功能一律**声明式**安装
  （只生成独立资产 + 挂 MA 的 `Merge Animator` / `Parameters` / `Menu Item` / `Menu Installer`），
  **不会**直接写你的 FX 控制器 / `ExpressionParameters` / `ExpressionsMenu`
...
- ⚠️ **自阴影只有烘焙路径**（②/④，读取烘焙的 `_SelfLightShadowMap`）。**实时自阴影（⑥）当前没有
  可用实现，其深度来源未决**（见 §6）；着色器侧不含"采样私有光 Unity 阴影贴图"的代码路径
...
- 性能等级与实时光源的额外开销（`Lights` 计数、阴影贴图、每个观看者各自的 ForwardAdd）：见 §5.2
- 常见坑：创建实时光源时需要自动开启 `Receive Shadows`（部分 avatar 默认关闭，关闭时没有影子）
- 自阴影的可用范围见 §6：**烘焙自阴影（②/④）可用，实时自阴影（⑥）当前没有可用实现**

**Quest / GLES3**

着色器本体、毛发与烘焙自阴影在 Quest 上的可用性见 §5.5；以下是本节补充的 GLES3 约束：

| 项 | 结论 |
|---|---|
| 采样器数量 | 安全，见 §5.5（去重后数量与 ES 3.0 规范下限） |
| `SampleLevel`、`ddx/ddy`、`Texture2DArray` | 均为 ES 3.0 核心能力 |
| 几何着色器 | GLES3 不支持。NonToonFur 的几何着色器 pass 数量见 §5.5，因此毛发仅限 PC |
...
**2026-09-18 起，两者是"并存"关系，不再是"升级"关系** —— 本分支已改用**自有包 id**
（`com.catandling.nontoon`）与**自有着色器名**（`nontoon-fork`）。以下为实测依据（2026-09）：

| 项 | 结果 |
|---|---|
| 包 id | **不同**（自有 `com.catandling.nontoon`）⇒ 不再覆盖官方包，两份会同时存在 |
| 着色器名 | **不同**（`nontoon-fork` / `nontoon-fork-fur` / `nontoon-fork-twopass`）⇒ 不会争同一个名字。<br>⚠️ 代价：按**名字**查找的代码（如 `Shader.Find("NonToon")`）与按名筛选资产的工具**不再命中本包**；按 GUID 引用的材质不受影响 |
| 资源 GUID | **与官方 0.1.3 逐字符相同**（共有的 78 个带 `.meta` 文件；`Shaders/NonToon.scshader` 为 `78361b0b760724141a2d2c09100cf00f`）⇒ 已有材质、预制体、场景**不会断引用**。⚠️ 正因如此，**官方版与本分支不能同时装**：同 GUID 的资产会被导入两次 |
...
| 起点 | 操作 |
|---|---|
| 全新安装 | 添加 listing → 只装 `NonToon (Fork)`（装工具包会自动带着色器） |
| 官方 0.1.3，或本分支 **0.3.10 及更早** | **先删掉旧的**（官方包，或 `Packages/jp.lilxyzw.nontoon`），**再**装本分支。包 id 不同 ⇒ VPM **不会**自动升级、也不会替你替换 |
| 本分支 0.1.10 及更早 | 该版本的着色包内嵌了转换器，覆盖安装可能残留两份，见第 7 节 |
| 0.3.6 及更早升级到 0.3.7 及以后 | 可能出现一次组件变为 Missing Script，见第 7 节 |

**回退**：本 listing 自 2026-09-18 起**只列出新身份的版本**（旧 id 的条目已移除），因此
**不能在 VPM 客户端里回退到 0.3.10 及更早**。那些版本的 zip 仍保留在仓库 `docs/` 下，
需要时可手动下载安装（安装前先删掉当前包）。
...
| 装置 A：AssetBundle 真编译 | 为目标平台实际编译着色器变体，并放入一个**故意写错的着色器作为负向对照**。对照未被报错则整轮结论无效 |
...
- 装置 A 必须在**找不到着色器时判失败**，并断言探针材质数量，否则包目录未同步时仍可能打印"全部通过"
- 探针必须在**运行开始先删除旧输出**，否则崩溃或超时后会残留上一轮的"全部通过"
- `Environment.ExitCode` 在 batchmode 下被忽略，探针必须用 `EditorApplication.Exit(code)`
- 发布校验必须做**三重一致性校验**（版本号 ↔ URL 文件名 ↔ zip 内 `package.json`）：若版本号取自
  `package.json`、而 zip 名另行硬编码，会出现"版本号与内容不一致仍发布成功"，
  且线上核验因哈希同源而判定一致
...
| 版本 | 要点 |
|---|---|
| 0.3.12 / 工具 0.5.4（**待发布**） | **待发布 —— 未在任何 listing 上提供，请勿当作已可用**。着色器侧：**T1 原生阴影外围增强（锐化 + 环境光染色）**（阶梯定义见[定位与能力阶梯](#定位与能力阶梯)）；**删除 `Shaders/Modules/SelfLight/NTRTDepth.shader`** —— 它唯一的调用方是工具包的 C# 半个（`Runtime/NTSelfRealtimeShadow.cs` 的 `Shader.Find("NonToon/NTRTDepth")`），该调用方随 0.5.4 一并移除，此后包内**不再有自渲深度代码**。工具包侧：**完全接入 MA / NDMF 构建期插件** —— 注册构建期 pass：**重放**创作期设置（固定亮度、① 光源的真实 `Light` 参数）后**消费掉创作期组件本身**（其产物保留：`Light` 组件、材质写入、MA 组件）、按 `_RenderingMode` 对齐**渲染器与动画替换材质**的队列、并对旧的非 MA 残留**明确报警**（不删用户资产）；新增直接依赖 `nadena.dev.ndmf`。相位：`Resolving`（早于 MA / AAO）与 `Transforming`（晚于 MA 的 late-transform-stages、早于 AAO 的优化阶段）。⚠️ **实时自阴影（⑥）仍然没有可用实现，深度来源未决**（见 §6）—— 本次删除的是**已死**的自渲深度文件与调用它的运行时灯架，不等于实时自阴影可用 |
| 0.3.11 / 工具 0.5.3 | **与上游彻底分割身份**：着色器包 id → `com.catandling.nontoon`，工具包 → `com.catandling.nontoon-converter`，着色器名 → `nontoon-fork` / `nontoon-fork-fur` / `nontoon-fork-twopass`；VPM 仓库 id → `io.github.catandling.vpm`（URL 未变）。**程序集名与资产 GUID 保留**（实测：与官方 0.1.3 共有的 78 个 `.meta` GUID 逐字符相同）⇒ 已有材质、预制体、场景不断引用；但**没有迁移路径**，一律按**全新安装**处理，且**装之前必须先删掉旧包**（同 GUID 的资产会被导入两次）。**着色器侧移除自渲线性深度 PCSS 路径**：15 个 `_SelfLightRt*` 属性与自渲采样代码已删（实测：0.3.10 的 zip 里 `_SelfLightRt` 命中 **34** 处，0.3.11 为 **0** 处）。⚠️ **实时自阴影在本版没有可用实现**：着色器侧挂钩 `_SelfLightRt*` 就是**在本版删掉**的，且着色器里**检索不到**任何"采样私有光 Unity 阴影贴图"的路径 ⇒ `SCOPE.md` 原先那句"实时自阴影设计未变、该路径什么都没被移除"**是假的，已撤回**（见 `NonToon/SCOPE.md`）。本版包内仍留着已死的 `Shaders/Modules/SelfLight/NTRTDepth.shader`，而工具包 `Runtime/NTSelfRealtimeShadow.cs` 仍在 `Shader.Find("NonToon/NTRTDepth")` 调用它（`SCOPE.md` 当时记明这一半「待移除」；二者已在 0.3.12 / 0.5.4 移除，见上一行）。工具包 **0.5.3**：身份改名 + **游戏内菜单去掉「光照下限」**（它 clamp 在最后一步，会把阴影对比整体压平；**4 项 / 25 bit → 3 项 / 17 bit**，仍 ≤30 预算）+ **[NT-FIX 31] 渲染队列兜底修复**（脚本/撤销/粘贴/第三方工具改 `_RenderingMode` 会让 `renderQueue` 陈旧，实测两个方向都错；新增命令按渲染模式重新对齐，幂等）+ **[NT-FIX 35] 缺失能力诊断修正**（按真实素材频次给专门、可定位的警告，不再用一条笼统文案宣称覆盖一切）+ 期望着色器版本对齐 0.3.11。另：**干净工程从零安装实测通过**（用 vrc-get 对 listing 真解析依赖，5 个包全解开） |
| 0.3.10 / 工具 0.5.2 | **0.3.10 起才带 `NTRTDepth.shader`** —— 0.3.9 的包里没有它，所以装了 0.3.9 的人 ⑥ 实时自阴影**根本跑不起来**（组件找不到 `NonToon/NTRTDepth` 会自己 disable），这是本次最要紧的一条；同时补 ⑥ 的整套 `_SelfLightRt*` 属性与 PCSS 通路（着色器侧）。工具包 **0.5.2**：**Modular Avatar 成为必需依赖**，需要菜单的功能一律**声明式**安装，旧的"直接写你的 FX 控制器 / ExpressionParameters / ExpressionsMenu"回退路径已整段删除（它曾在用户控制器里留下 `defaultWeight = 0` 的死层）。游戏内菜单改为**子菜单 + 4 个独立控制项**（光照下限 / 光照上限 / 阴影强度 / 阴影开关），合计 **25 bit**，不再是"一个不明不白的轮盘"；阴影开关**默认关**；两个区间按实机重标定（右上/弱端旧值实测"拉了没反应"）。灯架重复自愈。新增**环境光色温探测**：按 `ambientMode` 读环境色、**在着色器自己的色温曲线上反推**（往返精确 0 K），并如实告诉你偏绿/偏品红的环境色**是色温表达不了的**（避免"暖色地图里自己是一盏冷光"）|
| 0.3.9 / 工具 0.5.1 | 修复 **PCSS 软影宽度随烘焙分辨率变化**：半径以纹素定义、而包围盒与分辨率无关，导致 256²→512² 的世界宽度只剩 71%（1024² 只剩 57%）。判别实验证明漂移由**半径**驱动、且实际半影对纹素半径**超线性**（≈`R^1.55`）⇒ 按实测标定的 `g(s)=s^0.8`（`s = ShadowTexels/256`）换算半径与上下限，并同步按 `s²` 补采样数（封顶 64+32）。**256² 的输出与旧版逐像素一致**（实机实测最大差 1/255，存量 avatar 零变化），512²/1024² 的世界宽度已一致（实测 86%）。另修文档中「调高烘焙分辨率会更软」这条方向写反的建议 |
| 工具 0.5.0 | 新增 **④ 自带光照与阴影（不需要地图光源）**：组件自带虚拟光方向，一键把「私有光 + 亮度下限 + 烘焙自阴影」配好，实测虚拟光烘焙与真 Light 同方向**逐像素一致**；新增 **⑤ 亮度调整（构建时固定 / 游戏内可调）**：装了 Modular Avatar 就走**非破坏**路径（只生成独立资产 + 挂 MA 四个组件，不动你原有的控制器/参数/菜单，删组件即回滚）。另修「关掉自阴影仍会烘焙并挂贴图」的 bug |
| 0.3.8 / 工具 0.4.9 | 修复 PCSS 软阴影的取样点越界问题：深度图采样器的寻址模式实测是 Repeat（与贴图导入设置无关），未夹紧的取样点会读到深度图**对面**的纹素 —— 贴边一圈的阴影被冲淡（遮挡 1.000 → 0.642），对面是几何时还会凭空多出一条假阴影。现夹到纹素内部（只夹到 `[0,1]` 不够：Repeat 会把 `u = 1.0` 映射回纹素 0）。另更正文档中「PCSS 是固定循环」与「Quest 无动态循环」两处不实描述，补充软影宽度随烘焙分辨率变化等使用注意事项 |
