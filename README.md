...
3. 之后在工程的包管理页里会出现**两个**包（仓库里同时保留旧版本便于回退）：
   ...
   | 包 | 装不装 | 说明 |
   |---|---|---|
   | **`NonToon (Fork)` 0.3.4** | 必装 | 着色器本体，**纯库**（只有着色器 + 少量编辑器辅助） |
   | **`NonToon (Fork) Tools` 0.4.4** | 可选 | 工具包：材质转换器 + SelfLight（自有光源）烘焙器 / 实时光源 |
...
## 0.3.4 新增（把消耗压回去 —— NonToon 的本分）

**每像素自阴影采样预算**（默认全是"最省"那档）：

| 配置 | 采样/像素 |
|---|---|
| `_UseSelfLight = 0`（**默认**） | **0** |
| 自有光源开、强度 = 0（只要光不要影） | **0**（本轮新增短路：连 uv/包围盒都不算） |
| 自有光源开、PCSS 关（硬阴影） | **1** |
...
- 默认档位 **中(36) → 低(20)**；`_SelfLightReceiveMaskStrength` 默认 **1 → 0**。
- 强度为 0 时**一次采样都不做**（连包围盒/距离判断都跳过）。
- 组件面板直接显示「每像素自阴影采样：**20 次**（8 blocker + 12 PCF）；拉到 0 就是 **0 次**」。
- 这些默认值由探针**逐条断言**锁死，以后被改贵会直接报红。

> 对比一下代价：实时光源 / Avatar 光源插件在着色器侧是 0 采样，但 Unity 要**额外渲染一张阴影贴图**、
> 每个角色多一个 ForwardAdd，而且 **Lights = 1 → PC 上最高 Poor**，每个看到你的人都付这份开销。
> 要低消耗就用**烘焙阴影图**（默认那条）。
